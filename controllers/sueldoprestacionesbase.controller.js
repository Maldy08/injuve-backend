...
const COLLECTION_NAME = 'sueldoprestacionesbase';
...
exports.getSueldosPrestacionesBase = async (req, res) => {
    try {
        const db = getDb();
        const rows = await db.collection(COLLECTION_NAME).find({}).sort({ EMPLEADO: 1 }).toArray();
        res.json(rows);
    } catch (error) {
        console.error('Error al obtener sueldos y prestaciones base:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
};

exports.getSueldoPrestacionBaseById = async (req, res) => {
    ...
    try {
        ...
        console.error('Error al obtener registro base:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
};

exports.createSueldoPrestacionBase = async (req, res) => {
    ...
    try {
        ...
        console.error('Error al crear registro base:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
};

exports.updateSueldoPrestacionBase = async (req, res) => {
    const empleadoId = toNumber(req.params.id);
    if (empleadoId === null) {
        return res.status(400).json({ error: 'El id debe ser numérico' });
    }
    ...
    try {
        ...
        console.error('Error al actualizar registro base:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
};

exports.deleteSueldoPrestacionBase = async (req, res) => {
    const empleadoId = toNumber(req.params.id);
    if (empleadoId === null) {
        return res.status(400).json({ error: 'El id debe ser numérico' });
    }
    ...
    try {
        ...
        console.error('Error al eliminar registro base:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
};
